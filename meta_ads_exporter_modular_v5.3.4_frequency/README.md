# Meta Ads Exporter 5.3.3 by Asira
...
11. `Klik tautan`
12. `CTR klik tautan`
13. `Biaya per klik tautan`
14. `Hasil`
15. `Biaya per hasil`
16. `Jumlah yang dibelanjakan`
17. `CABANG`
18. `BISNIS`
19. `Tanggal proses`
...
XLSX mempertahankan kolom lengkap, termasuk ID set iklan, jam mulai, kategori waktu, `Frekuensi`, mata uang, `CABANG`, dan `BISNIS`. Token tidak masuk ke workbook.
...
## Definisi Hasil v5.3.3

Kolom `Hasil` hanya mengambil indicator Meta berikut:

```text
actions:onsite_conversion.messaging_conversation_started_7d
```

Jika indicator tersebut tidak terdapat pada field `results`, nilai `Hasil` dikosongkan. Tidak ada fallback ke metric lain.


## Frekuensi

Versi 5.3.4 mengekspor field `Frekuensi` pada mode `rekap` dan `rinci`, baik XLSX maupun CSV.

> Catatan analisis: `Frekuensi` adalah rasio, bukan metric aditif. Jangan menjumlahkan nilai frekuensi antar jam. Untuk frequency periode keseluruhan, gunakan mode rekap atau agregasi yang sesuai.
